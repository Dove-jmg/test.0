...
class U4HighLevelPolicy(nn.Module):
    """U4_upper的高层策略网络，专为4无人机系统设计"""
    ...
    def __init__(
        ...
        hidden_dim: int = 256,
        num_uavs: int = 4,  # 固定为4个无人机
        num_regions: int = 4,
        num_priority_levels: int = 3
    ):
        super(U4HighLevelPolicy, self).__init__()
        ...
        # 用于产生不同动作的头部网络
        self.assign_task_head = nn.Linear(hidden_dim, num_uavs)  # 分配任务给4个无人机
...
class U4LowLevelPolicy(nn.Module):
    """U4_upper的低层策略网络，为4无人机系统设计"""
    ...
    def __init__(
        ...
        super(U4LowLevelPolicy, self).__init__()
...
class U4HierarchicalRL:
    """U4_upper的分层强化学习管理器，专为4无人机系统设计"""
    
    def __init__(
        self,
        high_level_policy: U4HighLevelPolicy,
        low_level_policy: U4LowLevelPolicy,
        ...
        # 用于4无人机系统的特定参数
        self.uav_history = [[] for _ in range(4)]  # 每个无人机的历史任务记录
        
    def high_level_reward(
        self,
        assigned_value: float,
        total_value: float,
        balanced_assignments: float,  # 任务分配的平衡性指标
        high_delay: float,
        total_time: float
    ) -> float:
        """计算高层奖励函数 - U4版本"""
        ...
        # 任务分配平衡性奖励 (0-1之间，1表示完全平衡)
        balance_reward = self.high_level_weights[1] * balanced_assignments
        
        # 延迟惩罚
        delay_penalty = self.high_level_weights[2] * (high_delay / total_time)
        
        return assign_reward + balance_reward - delay_penalty
    ...
    def high_level_step(
        ...
        with torch.no_grad():
            ...
            # 采样具体动作 - 只考虑4个无人机
            assigned_uav = np.random.choice(min(len(assign_probs), 4), p=assign_probs[:4]/assign_probs[:4].sum())
    ...
    def low_level_step(
        ...
        with torch.no_grad():
            ...
            # 记录无人机的历史动作
            self.uav_history[uav_id].append(action)
    ...
    def store_high_level_transition(
        ...
        # 限制缓冲区大小
        if len(self.high_level_buffer) > 10000:
            self.high_level_buffer = self.high_level_buffer[-10000:]
    ...
    def store_low_level_transition(
        ...
        # 限制缓冲区大小
        if len(self.low_level_buffer) > 10000:
            self.low_level_buffer = self.low_level_buffer[-10000:]
    ...
    def get_uav_history(self, uav_id: int) -> List[Dict[str, Any]]:
        """获取指定无人机的历史记录"""
        if 0 <= uav_id < 4:
            return self.uav_history[uav_id]
        return []
    
    def clear_uav_history(self, uav_id: Optional[int] = None):
        """清除无人机历史记录"""
        if uav_id is not None and 0 <= uav_id < 4:
            self.uav_history[uav_id] = []
        else:
            self.uav_history = [[] for _ in range(4)]
