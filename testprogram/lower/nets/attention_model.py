...
from lower.utils.tensor_functions import compute_in_batches

from lower.nets.graph_encoder import GraphAttentionEncoder
from torch.nn import DataParallel
from lower.utils.beam_search import CachedLookup
from lower.utils.functions import sample_many
...
class AttentionModelFixed:
    ...
    def __getitem__(self, key):
        ...
        if self._recursion_guard:
            print("检测到递归调用AttentionModelFixed.__getitem__，返回None")
            return None
        ...
        try:
            if isinstance(key, int):
                ...
                return AttentionModelFixed(
                    node_embeddings=self.node_embeddings[key:key+1] if self.node_embeddings is not None else None,
                    context_node_projected=self.context_node_projected[key:key+1] if self.context_node_projected is not None else None,
                    glimpse_key=self.glimpse_key[:, key:key+1] if self.glimpse_key is not None else None,
                    glimpse_val=self.glimpse_val[:, key:key+1] if self.glimpse_val is not None else None,
                    logit_key=self.logit_key[key:key+1] if self.logit_key is not None else None
                )
            elif torch.is_tensor(key) or isinstance(key, slice):
                return AttentionModelFixed(
                    node_embeddings=self.node_embeddings[key] if self.node_embeddings is not None else None,
                    context_node_projected=self.context_node_projected[key] if self.context_node_projected is not None else None,
                ...
                raise TypeError(f"Expected key to be tensor, slice, or int, but got {type(key)}")
        except Exception as e:
            print(f"错误在AttentionModelFixed.__getitem__: {e}, key类型: {type(key)}")
            raise
        finally:
            # 完成后清除标记
            self._recursion_guard = False
