...
from U6_upper.utils.llm_integration import LLMInterface
from U6_upper.utils.hierarchical_rl import U6HighLevelPolicy, U6LowLevelPolicy
from U6_upper.utils.hierarchical_rl_impl import U6HierarchicalRL
from U6_upper.utils.dynamic_task_allocation_base import U6DroneState, U6TaskInfo
from U6_upper.utils.dynamic_task_allocation import U6DynamicTaskAllocator
from U6_upper.utils.task_integration import U6TaskIntegrationSystem
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("u6_integrated_run.log")
    ]
)
...
def run_integrated(args=None):
    ...
    """
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="U6_upper集成运行脚本")
    ...
    # 训练参数
    parser.add_argument('--num_regions', type=int, default=6, help="区域数量")
    ...
    # 路径参数
    parser.add_argument('--save_dir', type=str, default="./saved_models/u6", help="模型保存目录")
    parser.add_argument('--log_dir', type=str, default="./logs/u6", help="日志目录")
    parser.add_argument('--task_file', type=str, default=None, help="任务指令文件路径")
    
    # 系统配置参数
    parser.add_argument('--enable_collaboration', action='store_true', help="启用协作功能")
    parser.add_argument('--collaboration_ratio', type=float, default=0.3, help="需要协作的任务比例")
    parser.add_argument('--collaboration_benefit', type=float, default=1.5, help="协作带来的效益倍数")
    ...
    default_task_instructions = [
        """
        10:00 - 12:00，在校园东区收集空气质量数据，要求多点同步采集
        """,
        """
        10:00 - 12:00，在校园西区收集噪声数据，每40分钟采集一次
        """,
        """
        10:00 - 12:00，在校园南区收集道路流量数据，需要多架无人机协作
        """,
        """
        09:30 - 11:00，在居民区收集温度数据，每20分钟采集一次
        """,
        """
        10:30 - 12:00，在工业区收集空气污染数据，需要精确采样，多点同步
        """,
        """
        10:00 - 11:30，在商业区收集人流量数据，从多个角度协作监测
        """,
        """
        11:15 - 12:30，将收集的数据卸载至6个不同的边缘服务器，要求协调配合
        """
    ]
    ...
    # 初始化任务集成系统
    integration_system = U6TaskIntegrationSystem(
    ...
    if cli_args.integrate_original:
        ...
        # 生成协作策略示例
        if cli_args.enable_collaboration:
            logger.info("生成6无人机系统的协作策略示例...")
            strategy = integration_system.generate_collaboration_strategy(
                drones=[0, 1, 2, 3, 4, 5],
                task_description="在校园区域进行全面的环境监测，包括空气质量、噪声水平和道路流量"
            )
            
            logger.info(f"生成的协作策略: {json.dumps(strategy, indent=2, ensure_ascii=False)}")
    ...
    results = {
        ...
        "collaboration_enabled": cli_args.enable_collaboration,
        "integrated_with_original": cli_args.integrate_original
    }
    
    with open(os.path.join(cli_args.save_dir, "u6_results.json"), "w") as f:
        json.dump(results, f, indent=2)
    
    logger.info(f"结果已保存到 {os.path.join(cli_args.save_dir, 'u6_results.json')}")
