...
import warnings
from lower.utils import load_model as Lload_model
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    Lmodel, _ = Lload_model(opts.Lmodel)
    ...
    if opts.multiprocessing:
        ...
        with mp.Pool(num_processes) as pool:
            a = [(dataset_path, width, softmax_temp, opts, i, num_processes) for i in range(num_processes)]
            b = pool.map(eval_dataset_mp, a)
        ...
        results = _eval_dataset(model, Lmodel, dataset, width, softmax_temp, opts, device)
...
def _eval_dataset(model, Lmodel, dataset, width, softmax_temp, opts, device):
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        with torch.no_grad():
            if opts.decode_strategy in ('sample', 'greedy'):
                ...
                sequences, costs, veh_lists = model.sample_many(Lmodel, batch, batch_rep=batch_rep, iter_rep=iter_rep)
                print('cost', costs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, help='The dir of the higher-layer model')
    parser.add_argument('--Lmodel', type=str, help='The dir of the lower-layer model')
    ...
    # the dir of the testing datasets
    opts.datasets = [r'./data/taop_80_seed1234.pkl']
